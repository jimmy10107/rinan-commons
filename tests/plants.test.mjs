...
test('static page and shared navigation expose a proper standalone plants route',()=>{
 const h=readFileSync('out/plants/index.html','utf8');assert.match(h,/<h1[^>]*>日南植物形態室<\/h1>/);assert.match(h,/植物生長影格動畫/);assert.match(h,/frames\/taro_01.svg/);assert.match(h,/生長影格/);assert.doesNotMatch(h,/<canvas/);
...
test('all thirty playback frames are real lightweight vectors',()=>{
 let total=0;for(const p of plants)for(let i=1;i<=5;i++){const s=readFileSync(`public/plants/frames/${p.id}_${String(i).padStart(2,'0')}.svg`,'utf8');assert.match(s,/<path/);assert.doesNotMatch(s,/<image|<script|data:image|foreignObject/);assert.ok(Buffer.byteLength(s)<180000);total+=Buffer.byteLength(s);}assert.ok(total<2200000);
});
