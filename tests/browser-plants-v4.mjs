...
const out=path.resolve('out'),qa='docs/qa/plants-v5';mkdirSync(qa,{recursive:true});
...
try{
 ...
 await p.getByRole('button',{name:'重播生長動畫',exact:true}).click();await p.locator('.plant-film').waitFor();const begin=Date.now();await p.waitForFunction(()=>document.querySelector('.plant-observatory')?.getAttribute('data-intro')==='false');results.filmMs=Date.now()-begin;assert.equal(await p.locator('.plant-film-release').count(),1);await p.waitForFunction(()=>document.querySelector('.plant-observatory')?.getAttribute('data-handoff')==='false');await ready('models/taro-5-lite.glb');results.smoothHandoff=true;assert.ok(results.filmMs>=11700&&results.filmMs<13500);
