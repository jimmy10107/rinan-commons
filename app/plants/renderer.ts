...
export function createPlantViewer(host: HTMLElement, report: (state: 'loading'|'ready'|'error')=>void) {
  ...
  const scene=new THREE.Scene();scene.background=new THREE.Color('#17231c');
  ...
  let last=0,slowFrames=0,movingFrames=0;
  ...
  async function load(url:string){
    const current=++version;abort?.abort();abort=new AbortController();report('loading');canvas.dataset.ready='false';
    if(model){scene.remove(model);disposeModel(model);model=null;}invalidate();
    try{
      const response=await fetch(url,{signal:abort.signal});if(!response.ok)throw Error('Model unavailable');
      const bytes=await response.arrayBuffer();if(dead||current!==version)return;
      const gltf=await loader.parseAsync(bytes,'');
      if(dead||current!==version){disposeModel(gltf.scene);return;}
      model=gltf.scene;scene.add(model);const box=new THREE.Box3().setFromObject(model),sphere=box.getBoundingSphere(new THREE.Sphere());center=box.getCenter(new THREE.Vector3());radius=sphere.radius;dim=box.getSize(new THREE.Vector3()).length();
      camera.near=dim*.0001;camera.far=dim*80;controls.minDistance=radius*.12;controls.maxDistance=radius*30;view();
      // PBR programs may compile asynchronously: keep the preview until the first real frame.
      ...
      if(dead||current!==version)return;
      renderer.render(scene,camera);renderer.getContext().flush();
      canvas.dataset.ready='true';canvas.dataset.model=url;report('ready');invalidate();
    }catch(e){if(!dead&&current===version&&(e as Error).name!=='AbortError')report('error');}
  }
  ...
  return {load,view,zoom,rotate(on:boolean){controls.autoRotate=on;invalidate();},backlight(on:boolean){rim.intensity=on?4:1.6;key.intensity=on?.85:2.5;invalidate();},snapshot(){renderer.render(scene,camera);return new Promise<Blob|null>(resolve=>canvas.toBlob(resolve,'image/png'));},
