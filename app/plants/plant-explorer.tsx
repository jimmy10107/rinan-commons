...
const FILM_DURATION=12000, FRAME_DURATION=2400, HANDOFF_DURATION=800;
...
export default function PlantExplorer(){
 ...
 const [handoff,setHandoff]=useState(false),[playing,setPlaying]=useState(true),[frame,setFrame]=useState(0),[elapsed,setElapsed]=useState(0),[replay,setReplay]=useState(0),[warmed,setWarmed]=useState<string[]>([]);
 ...
 useEffect(()=>{
  ...
  function tick(now:number){const delta=Math.max(0,now-last);last=now;if(!document.hidden){time+=delta;setElapsed(Math.min(time/FILM_DURATION,1));setFrame(Math.max(0,Math.min(4,Math.floor(time/FRAME_DURATION))));}if(time<FILM_DURATION)raf=requestAnimationFrame(tick);else {setHandoff(true);setPlaying(false);}}
 ...
 function select(i:number){setHandoff(false);setIndex(i);setWarmed([]);setFrame(0);setElapsed(0);setReplay(n=>n+1);setPlaying(!matchMedia('(prefers-reduced-motion: reduce)').matches);setDark(i===4);setMode('whole');setStage(4);setZoomed(false);setShown('');setRotating(false);setBacklit(false);setReferences(false);setQuality('auto');const u=new URL(location.href);u.searchParams.set('plant',assets[i].id);u.searchParams.delete('stage');u.searchParams.delete('mode');history.replaceState(null,'',u);}
 function selectStage(s:number){setHandoff(false);setPlaying(false);setQuality('auto');setStage(s);setMode('whole');setZoomed(false);setShown('');setRotating(false);const u=new URL(location.href);u.searchParams.set('plant',plant.id);u.searchParams.set('stage',String(s+1));u.searchParams.delete('mode');history.replaceState(null,'',u);}
 function stop(){setHandoff(false);setPlaying(false);setActive(false);setStatus('idle');setRotating(false);setBacklit(false);}
 ...
 useEffect(()=>{
  if(!handoff||playing||!modelReady)return;
  const timer=setTimeout(()=>setHandoff(false),matchMedia('(prefers-reduced-motion: reduce)').matches?0:HANDOFF_DURATION);
  return()=>clearTimeout(timer);
 },[handoff,playing,modelReady]);
 const ready=modelReady&&!playing;
 const warmCount=plant.stages.filter(s=>warmed.includes(s.lite.file)).length;
 return <section data-intro={playing} data-handoff={handoff} data-warmed={warmCount} className="plant-observatory" aria-label="植物模型探索">
  ...
  <div className="plant-workbench">
   <div className="plant-visual-column">
    <div className="plant-scene" data-state={status} data-dark={dark}>
     <div ref={host} className="plant-canvas-host" style={{opacity:playing||!modelReady?0:1,pointerEvents:playing?'none':undefined}} hidden={!active||status==='error'}/>
     {!ready&&!playing&&!handoff&&<img className="plant-poster" src={poster} srcSet={mode==='macro'?`${assetURL(`previews/${plant.id}-480.webp`)} 480w, ${poster} 960w`:undefined} sizes="(max-width:760px) 100vw, 65vw" alt={`${plant.name}｜${mode==='macro'?'器官特寫':plant.stages[stage].label}的三維模型渲染預覽`} width={960} height={960} fetchPriority="high"/>}
     {(playing||handoff)&&<div className={`plant-film${!playing&&modelReady?' plant-film-release':''}`} aria-label="植物生長影格動畫">{plant.stages.map((s,i)=><img key={s.label} src={frameURL(i)} className={i===frame?'current':''} alt={i===frame?`${plant.name}・${s.label}`:''} aria-hidden={i!==frame} width={1000} height={1040} fetchPriority={i===0?'high':'auto'}/>)}</div>}
     ...
     <span className="plant-image-caption">{playing?'約 12 秒，慢慢走過五個生長姿態':ready?'拖曳旋轉 · 雙指縮放':'植物形態靜態影格'}</span>
    </div>
    ...
    <div className="plant-film-replay"><button onClick={()=>{setHandoff(false);setMode('whole');setStage(4);setQuality('auto');setZoomed(false);setRotating(false);viewer.current?.view('front');setFrame(0);setElapsed(0);setReplay(n=>n+1);setPlaying(true);}}>重播生長動畫</button><span>動畫為五個觀察姿態，並非等長生長時間。</span></div>
