...
const places = [
  ["日南車站", "日南車站 台中大甲"], ["稻站・日南文化前哨站", "日南車站 兵仔營"],
  ...
  return (
    <main id="top">
      ...
      <header className="topbar">
        <a className="brand" href="#top" aria-label="走傱日南首頁"><span className="brand-logo"><img src="/partners/rinan-commons.jpg" alt="" /></span><span><strong>走傱日南</strong><small>活動與地方指南</small></span></a>
        <span className="event-date">10.24—25</span>
      </header>
      ...
      <footer><strong>日南稻站 Rinan Commons</strong><span>日南車站旁・資料版本 2026.09.03</span><small>活動內容以最終公告為準</small></footer>
...
function LocalInfo() {
  return <>
    ...
    <details className="info-card">
      <summary><span className="card-icon">冊</span><div><small>TO AND FROM</small><strong>《往返》策展手冊</strong><em>閱讀四個章節</em></div><i>＋</i></summary>
  ...
  return <section className="credit-section">
    ...
    <div className={`organization-grid${organizer ? " organizers" : ""}`}>
      {items.map((item, index) => {
        ...
        return item.href
          ? <a key={item.name} href={item.href} target="_blank" rel="noreferrer">{contents}</a>
